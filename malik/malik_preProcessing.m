...
figure, imshow(rSegmented), title('RSegmented'); 

gSegmented((hsi(:,:,1)>0.4 & hsi(:,:,1)<0.5)&(hsi(:,:,2)>0.7)&(hsi(:,:,3)>0.5)) = 1;
figure, imshow(gSegmented), title('GSegmented'); 

bSegmented((hsi(:,:,1)>0.33 & hsi(:,:,1)<0.6)&(hsi(:,:,2)>0.7)&(hsi(:,:,3)>0.3)) = 1;
figure, imshow(bSegmented), title('BSegmented'); 

ySegmented((hsi(:,:,1)>0.1 & hsi(:,:,1)<0.2)&(hsi(:,:,2)>0.9)&(hsi(:,:,3)>0.8)) = 1;
figure, imshow(ySegmented), title('YSegmented'); 
end